...
export async function exampleExplicitWait(driver) {
  ...
  let element = await driver.wait(until.elementLocated(By.id('myElementId')), 10000);
  await driver.wait(until.elementIsVisible(element), 5000);

  // Wait for an element to be clickable
  let clickableElement = await driver.wait(until.elementToBeClickable(By.css('.myButtonClass')), 10000);
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (error.name === "StaleElementReferenceError") {
        console.warn(`Stale element reference encountered, retrying... (${i + 1}/${maxRetries})`);
